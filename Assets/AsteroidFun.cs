...
public class AsteroidFun : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        GameObject[] asteroidOnScreen = GameObject.FindGameObjectsWithTag("Asteroid");
        if (collision.gameObject.tag == "PlayerRocket")
        {
            foreach (GameObject asteroid in asteroidOnScreen)
            {
                if (Vector2.Distance(asteroid.transform.position,transform.position) <= 3f)
                {
                    Destroy(collision.gameObject);
                    Destroy(asteroid);
                    RocketPhysics.numberOfAsteroids--;
