...
public class RocketPhysics : MonoBehaviour
{
    ...
    public static float coolDownPeriodInSeconds = 2f;
    ...
    void Update()
    {
        ...
        if (timeStamp <= Time.time)
        {
            if (Input.GetKeyDown(KeyCode.F))
            {
                ...
                rocket.GetComponent<Rigidbody2D>().AddForce(rocket.transform.up * 100f);
                Destroy(rocket, 1f);
            }
        }
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (objectTag != "PlayerBullet" && objectTag != "PlayerRocket")
        {
